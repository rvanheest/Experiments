...
object Testing extends App {
  ...
  implicit val consoleTestIO: Console[TestIO] = new Console[TestIO] {
    override def println(msg: ConsoleOut): TestIO[Unit] = {
      for {
        ...
        setted <- TestIO.put(testData.copy(output = msg :: testData.output))
      } yield setted
    }
  ...
  val TestData(inputResult, outputResult, randomsResult) = Game.runApplication[TestIO].exec(testData)
