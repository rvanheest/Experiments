 ...
 */
package com.github.rvanheest.starbux.barista.service
...
trait BaristaServerComponent {
  this: BaristaServletMounterComponent with DebugEnhancedLogging =>

  val server: BaristaServer

  class BaristaServer(serverPort: Int) {
    ...
    def start(): Try[Unit] = Try {
      logger.info("Starting Barista server...")
      server.start()
    }

    def stop(): Try[Unit] = Try {
      logger.info("Stopping Barista server...")
      server.stop()
    }

    def destroy(): Try[Unit] = Try {
      server.destroy()
      logger.info("Barista server stopped.")
    }
  }
}
